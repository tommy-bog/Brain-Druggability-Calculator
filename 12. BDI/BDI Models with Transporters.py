...
rdkitdesc = RdkitDescriptors('Data.csv', 'BDI', 'SMILES')
df = rdkitdesc.rdkit_fingerprints()

fps = np.empty((41, 2048))
...
for i in range(0,90):
      ...
      avgR2_5_fold_CV = st.mean(scoresR2)
      r_squared_CV_vals[i] = avgR2_5_fold_CV
# 52 bits =  79 (85,58), 35 (89,54) , 70 (72,74) , (74,66) 



X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=y, random_state=79)


model = GaussianProcessRegressor(kernel=kernel, random_state=1, alpha=1e-7, n_restarts_optimizer=4, normalize_y=True).fit(X_train, y_train)
...
y_train = y_train.ravel()
scoresR2 = cross_val_score(model, X_train, y_train, cv=4, scoring = 'r2')
scoresRMSE = cross_val_score(model, X_train, y_train, cv=4, scoring = 'neg_root_mean_squared_error')
